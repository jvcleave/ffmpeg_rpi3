...
configure: 
	...
	LDFLAGS="" \
  ./configure \
			--extra-cflags="-mfpu=vfpv3-d16 -mfloat-abi=$(FLOAT) -mno-apcs-stack-check -mstructure-size-boundary=32 -mno-sched-prolog" \
...
.PHONY : install
install:
	cd ffmpeg; make -j2 DESTDIR="$(WORK)/ffmpeg_compiled" install
